...
from bray_app.services.valvesize_service import get_all_valvesize, get_all_valvetype, get_enabled_categories, delete_multiple_valvesize
...
def edit_valve_list(request, size_id):
    ...
    if request.method == "GET":
        with connection.cursor() as cursor:
            ...
            row = cursor.fetchone()
            if row:
                edit_valve = dict(zip(["id", "size_id", "name", "description"], row))
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if duplicate:
                return JsonResponse({
                    "status":"error",
                    "message":"Valve size name is already exists.Duplicate names are not allowed"
                }, status=409)
            

            
            cursor.execute("SELECT COALESCE(MAX(SIZE_ID), 0) + 1 FROM valvesize FOR UPDATE")
            ...
            with connection.cursor() as cursor:
                ...
                if frontend_ids:
                    cursor.execute("""
                            ...
                            """ % ("%s", ",".join(["%s"] * len(frontend_ids))), [size_id] + frontend_ids)
                else:
                    ...
                    if standard_og_id :
                        ...
                        cursor.execute(f"""
                            UPDATE master_duration_data 
                                ...
                                """,
                                [standard_id] + values  + [standard_og_id , size_id]
                        )
                    else:
                        col_names = ", ".join(duration_cols.keys())
                        placeholders = ", ".join(["%s"] * len(duration_cols))
